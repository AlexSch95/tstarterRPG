        ...
        while wave_count > 0 and self.health_points > 0:
            ...
            while True:
                ...
                if attack_y_n.upper() == "J":
                    enemy_id.enemy_hp -= self.weapon.weapon_damage  # Mein verursachter Schaden wird von den HP des Gegners abgezogen
                    print(
                        f"Du hast den {enemy_id.enemy_name} angegriffen und {self.weapon.weapon_damage} Schaden verursacht.")
                    print(f"Der {enemy_id.enemy_name} hat nun {enemy_id.enemy_hp}HP")
                    if enemy_id.enemy_hp <= 0:
                        print(f"Du hast den {enemy_id.enemy_name} besiegt.")
                        self.gold += enemy_id.enemy_bounty
                    ...
                    print(f"Deine Lebenspunkte sinken auf {self.health_points}/{self.maxhp} Healthpoints")
                    if self.health_points <= 0:
                        print(f"Du bist gestorben. Kehre zum Hauptmenü zurück")
                        break
                    if self.health_points <= 25:
                        while True:
                            heal_y_n = input(
                                "Deine HP sind gering, Möchtest du dich Heilen vor dem nächsten Angriff? (J/N)")
                            if heal_y_n.upper() == "J":
                                self.use_potion()
                                break
                            elif heal_y_n.upper() == "N":
                    ...
                    print(
                        f"Du hast den Kampf abgebrochen, der {enemy_id.enemy_name} ist mit {enemy_id.enemy_hp}HP entkommen")
                    print()
                    break
                else:
                    continue"""
            ...
            if wave_count == 0:
                print(f"Du hast alle {wave_count} Wellen besiegt, Glückwunsch!")
